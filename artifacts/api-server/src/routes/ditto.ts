...
// Track last worker poll time (updated by ditto_worker.js via /api/jobs/pending)
...
router.get("/user/:uid", async (req, res) => {
  ...
  try {
    ...
    if (giftsResult && (giftsResult as Record<string, unknown>).code === 200) {
      ...
      topGifts = (data?.topList as unknown[]) ?? [];
...
router.get("/worker/status", (req, res) => {
  // Count pending jobs
  const pendingJobs = 0; // simplified — jobs map is in the other router
